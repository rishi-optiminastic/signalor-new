...
const gscSitesResponseSchema = z.object({ sites: z.array(gscSiteSchema) })
...
const gscPageMatchSchema = z.object({
  found: z.boolean(),
  page: z.string(),
  clicks: z.number(),
  impressions: z.number(),
...
const gscDataSnapshotSchema = z.object({
  id: z.number(),
  date_start: z.string(),
  date_end: z.string(),
  clicks: z.number(),
  ...
  ctr: z.number(),
  position: z.number(),
  daily_trend: z.array(gscRowSchema.extend({ date: z.string() })),
  top_queries: z.array(gscRowSchema.extend({ query: z.string() })),
  top_pages: z.array(gscRowSchema.extend({ page: z.string() })),
  countries: z.array(gscRowSchema.extend({ country: z.string() })),
  sync_status: syncStatusSchema,
  error_message: z.string(),
  created_at: z.string(),
  page_match: gscPageMatchSchema.optional(),
})

const gscInspectionSchema = z.object({
  inspected_url: z.string(),
  on_google: z.boolean(),
  verdict: z.string(),
  coverage_state: z.string(),
  robots_txt_state: z.string(),
  indexing_state: z.string(),
  last_crawl_time: z.string(),
  page_fetch_state: z.string(),
  google_canonical: z.string(),
  user_canonical: z.string(),
  crawled_as: z.string(),
})

const gscSitemapEntrySchema = z.object({
  path: z.string(),
  type: z.string(),
  is_index: z.boolean(),
  is_pending: z.boolean(),
  last_submitted: z.string(),
  last_downloaded: z.string(),
  warnings: z.number(),
  errors: z.number(),
  submitted: z.number(),
  indexed: z.number(),
...
const gscSitemapsSchema = z.object({
  sitemaps: z.array(gscSitemapEntrySchema),
  submitted: z.number(),
  indexed: z.number(),
})

const gscCoveragePageSchema = z.object({
  url: z.string(),
  clicks: z.number(),
  impressions: z.number(),
  ctr: z.number(),
  position: z.number(),
  coverage_state: z.string().default(''),
})

const gscNotIndexedPageSchema = z.object({
  url: z.string(),
  reason: z.string().default(''),
})

const gscCoverageSchema = z.object({
  property: z.string().default(''),
  sync_status: z.enum(['pending', 'syncing', 'complete', 'failed']).default('complete'),
  checked_at: z.string().default(''),
  date_start: z.string().default(''),
  date_end: z.string().default(''),
  indexed_count: z.number().default(0),
  not_indexed_count: z.number().default(0),
  checked_count: z.number().default(0),
  sitemap_total: z.number().default(0),
  submitted: z.number().default(0),
  pages: z.array(gscCoveragePageSchema).default([]),
  not_indexed: z.array(gscNotIndexedPageSchema).default([]),
})
...
export type GSCDataSnapshot = z.infer<typeof gscDataSnapshotSchema>
export type GSCInspection = z.infer<typeof gscInspectionSchema>
export type GSCSitemapEntry = z.infer<typeof gscSitemapEntrySchema>
export type GSCSitemaps = z.infer<typeof gscSitemapsSchema>
export type GSCCoverage = z.infer<typeof gscCoverageSchema>
export type GSCCoveragePage = z.infer<typeof gscCoveragePageSchema>
...
export async function getGSCAuthUrl(
  ...
  const frontendBase = typeof window !== 'undefined' ? window.location.origin : undefined
  const { data } = await apiClient.get('/api/integrations/google-search-console/auth-url/', {
    params: { email, return_to: returnTo, frontend_base: frontendBase },
  })
  return authUrlSchema.parse(data)
}

export async function disconnectGSC(email: string): Promise<{ message: string }> {
  const { data } = await apiClient.delete('/api/integrations/google-search-console/disconnect/', {
    params: { email },
  })
  return messageOnlySchema.parse(data)
}

export async function getGSCSites(email: string): Promise<{ sites: GSCSite[] }> {
  const { data } = await apiClient.get('/api/integrations/google-search-console/sites/', {
    params: { email },
  })
  return gscSitesResponseSchema.parse(data)
}
...
export async function selectGSCSite(
  ...
  const { data } = await apiClient.post('/api/integrations/google-search-console/select-site/', {
    email,
    site_url: siteUrl,
  })
  return integrationActionSchema.parse(data)
...
export async function syncGSCData(email: string): Promise<{ message: string }> {
  const { data } = await apiClient.post('/api/integrations/google-search-console/sync/', null, {
    params: { email },
  })
  return messageOnlySchema.parse(data)
}

export async function getGSCData(email: string, analyzedUrl?: string): Promise<GSCDataSnapshot> {
  const { data } = await apiClient.get('/api/integrations/google-search-console/data/', {
    params: { email, analyzed_url: analyzedUrl },
  })
  return gscDataSnapshotSchema.parse(data)
}

export async function inspectGSCUrl(email: string, url: string): Promise<GSCInspection> {
  const { data } = await apiClient.get('/api/integrations/google-search-console/inspect/', {
    params: { email, url },
  })
  return gscInspectionSchema.parse(data)
}

export async function getGSCSitemaps(email: string): Promise<GSCSitemaps> {
  const { data } = await apiClient.get('/api/integrations/google-search-console/sitemaps/', {
    params: { email },
  })
  return gscSitemapsSchema.parse(data)
}

export async function getGSCCoverage(email: string): Promise<GSCCoverage> {
  const { data } = await apiClient.get('/api/integrations/google-search-console/coverage/', {
    params: { email },
  })
  return gscCoverageSchema.parse(data)
}
...
const blogTopicsResponseSchema = z.object({
  topics: z.array(blogTopicSchema).default([]),
  has_gsc: z.boolean().default(false),
  has_ga: z.boolean().default(false),
})

export type BlogTopic = z.infer<typeof blogTopicSchema>
export type BlogTopicsResponse = z.infer<typeof blogTopicsResponseSchema>

/** AI blog topic ideas grounded in GSC keywords + GA + tracked prompts. */
export async function getBlogTopics(
  runSlug: string,
  opts?: { refresh?: boolean },
): Promise<BlogTopicsResponse> {
  const { data } = await apiClientLong.get(`/api/analyzer/runs/s/${runSlug}/blog/topics/`, {
    params: opts?.refresh ? { refresh: 1 } : undefined,
    timeout: 60_000,
  })
  return blogTopicsResponseSchema.parse(data)
}
...
export async function uploadBlogImage(runSlug: string, file: File): Promise<{ url: string }> {
  const form = new FormData()
  form.append('file', file)
  const { data } = await apiClient.post(
    `/api/analyzer/runs/s/${runSlug}/blog/upload-image/`,
    form,
    { headers: { 'Content-Type': 'multipart/form-data' } },
  )
  return z.object({ url: z.string(), id: z.number().optional() }).parse(data)
}
