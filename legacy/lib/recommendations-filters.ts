...
export type RecommendationSourceFilter = 'all' | 'ai_insight' | 'analyzer'

export const RECOMMENDATION_SOURCE_LABELS: Record<RecommendationSourceFilter, string> = {
  all: 'All sources',
...
export function filterRecommendations(
  ...
  sourceFilter: RecommendationSourceFilter = 'all',
): Recommendation[] {
  ...
  return items.filter(rec => {
    ...
    if (sourceFilter !== 'all' && (rec.source ?? 'analyzer') !== sourceFilter) return false
