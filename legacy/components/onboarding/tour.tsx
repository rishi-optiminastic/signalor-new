...
import { getProfile, updateProfile } from "@legacy/lib/api/profile";
...
export function TourProvider({
  ...
  // DB-backed "has seen the product tour" flag — the source of truth, so the
  // tour doesn't re-trigger on a new browser/device/cache-clear. null = still
  // loading; we only auto-start when it's a definite `false`.
  const [hasSeenTour, setHasSeenTour] = React.useState<boolean | null>(null);
  React.useEffect(() => {
    if (!email) return;
    let cancelled = false;
    getProfile(email)
      .then((p) => {
        if (!cancelled) setHasSeenTour(Boolean(p.has_seen_product_tour));
      })
      .catch(() => {
        // Fail open: localStorage still prevents per-section repeats this browser.
        if (!cancelled) setHasSeenTour(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  React.useEffect(() => {
    ...
    // DB flag is authoritative: only auto-start when we KNOW it's unseen.
    // (null while loading, true once dismissed/finished → never auto-start.)
  ...
  }, [autoStart, currentSection, sectionSteps.length, hasSeenTour]);
  ...
  const markCompleted = React.useCallback(() => {
    ...
    // Skip or Finish → durably suppress the tour for this user everywhere.
    setHasSeenTour(true);
    if (email) {
      updateProfile(email, { has_seen_product_tour: true }).catch(() => {});
    }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 px-4 pb-3 pt-4">
        ...
        <h3 className="text-sm font-semibold leading-tight text-foreground">{step.title}</h3>
