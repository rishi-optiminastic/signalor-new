...
import { getGAData, syncGAData, type GADataSnapshot } from "@legacy/lib/api/integrations";
...
export function GATrafficTab({ email, analyzedUrl }: GATrafficTabProps) {
  ...
  if (loading) {
    return (
      <p className="text-sm text-muted-foreground py-8 text-center">Loading traffic data...</p>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">Last updated {ago}</p>
      ...
      {analyzedUrl && data.page_match && (
        ...
        >
          {data.page_match.found ? (
            <>
              Exact page match confirmed:{" "}
              <span className="font-mono">{data.page_match.page_path}</span> • Sessions:{" "}
              {data.page_match.sessions}
            </>
          ) : !data.page_match.host_match ? (
            <>
            ...
            <>
              Exact analyzed page not found in GA data window for{" "}
              <span className="font-mono">{data.page_match.page_path}</span>.
            </>
          )}
        </div>
      )}
