...
import type { LucideIcon } from "@legacy/components/icons";
import {
  ArrowUpRight,
  BarChart3,
  Bell,
...
import { LANDING_PRIMARY_CTA_CLASS } from "@legacy/components/landing/constants";
  ...
  solutions: {
    ...
    items: [
      ...
      },
      {
        ...
        title: "Visibility",
        desc: "Track AI mentions and citations.",
        icon: BarChart3,
      },
      {
        href: "/solutions/fix-playbook",
        title: "Fix Playbook",
        desc: "Prioritized GEO fixes to ship.",
        icon: Target,
      },
      {
        href: "/solutions/competitive-lens",
        title: "Competitive Lens",
        desc: "Benchmark citations against rivals.",
        icon: LineChart,
      },
    ],
  },
    ...
    items: [
      {
        ...
        desc: "Track prompts that surface your brand.",
        icon: LayoutGrid,
      },
      {
        href: "/ai-visibility",
        title: "AI visibility",
        desc: "Score your brand's AI presence.",
        icon: Eye,
      },
      {
        href: "/recommendations",
        title: "Recommendations",
        desc: "Impact-ranked GEO fixes.",
        icon: Plug,
      },
      {
        href: "/explorer",
        title: "Explorer",
        desc: "Discover prompts and competitor gaps.",
        icon: BarChart3,
      },
    ],
  },
    ...
    items: [
      {
        ...
        title: "URL analyzer",
        desc: "Score any URL's GEO health.",
        icon: Link2,
      },
      {
        href: "/tools/llms-check",
        title: "LLM checker",
        desc: "See what AI says about you.",
        icon: Bot,
      },
      {
        href: "/tools/competitors-analysis",
        title: "Competitors analysis",
        desc: "Compare your AI citation share.",
        icon: Users,
      },
      {
        href: "/tools/schema-validator",
        title: "Schema validator",
        desc: "Validate your JSON-LD markup.",
        icon: Binary,
      },
    ],
  },
...
// Every primary destination, de-duplicated by href. The interactive mega-menu
// only mounts its links on hover/click (client-side), so crawlers never see
// them. We render this list as a server-side, visually-hidden <nav> (see
// CrawlableNav) so search engines discover the whole site at shallow depth —
// improving internal link equity, crawl coverage, and sitelink eligibility.
const PRIMARY_NAV_LINKS: { href: string; title: string }[] = [
  ...Object.values(MENUS).flatMap((menu) =>
    menu.items.map((item) => ({ href: item.href, title: item.title })),
  ),
...
function MegaMenuLinkCell({
  ...
  return (
    ...
    >
      <Link
        href={item.href}
        className="group flex gap-3 rounded-none border border-transparent p-2 text-left transition-colors hover:bg-muted/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-1"
        onClick={onNavigate}
      >
        <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-none border border-black/8 text-primary shadow-sm">
...
function ResourcesMegaGrid({
  ...
  return (
    <motion.div
      className="grid gap-2 rounded-none border border-black/6 sm:h-64 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(220px,260px)] sm:grid-rows-2 sm:gap-x-3 sm:gap-y-1 justify-center"
  ...
  return (
    ...
    >
      <Link
        ...
        href={item.href}
        className={cn(
          "flex h-full flex-col justify-center rounded-none border p-2 transition-colors",
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-1",
          isActive ? "border-primary/20 bg-primary/5" : "border-transparent hover:bg-muted/80",
        )}
      ...
      >
        <span className="flex gap-3 text-left">
          <span
            className={cn(
              "flex h-12 w-12 shrink-0 items-center justify-center rounded-none border shadow-sm transition-colors",
              isActive
...
function ResourcePreviewPanel({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden rounded-none border border-black/8 bg-muted/80 p-2 shadow-inner">
...
function PreviewHeading({ label }: { label: string }) {
  return (
    <span className="px-1 pb-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">
      {label}
    </span>
  );
}
        ...
        {INTEGRATION_PLUGINS.map((p) => (
          <li key={p.key} className="flex flex-1">
            <Link
              ...
              onClick={onNavigate}
            >
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-none border border-black/8 bg-white shadow-sm">
        ...
        {loading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <li key={i} className="flex items-start gap-2 rounded-md p-2">
              <span className="mt-0.5 h-5 w-5 shrink-0 animate-pulse rounded-none bg-muted" />
          ...
          <li className="px-2 py-3 text-xs text-muted-foreground">No posts published yet.</li>
        ) : (
          posts.map((post) => (
            <li key={post.slug}>
              <Link
                ...
                onClick={onNavigate}
              >
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-none border border-black/8 bg-white text-primary shadow-sm">
                  <FileText className="h-3 w-3" strokeWidth={1.75} aria-hidden />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[13px] font-semibold tracking-tight text-foreground transition-colors group-hover:text-primary">
                    {post.title}
                  </span>
...
function LoginPreview({
  ...
  return (
    <>
      ...
      <ul role="list" className="flex flex-col gap-0.5">
        {LOGIN_LINKS.map((link) => (
          <li key={link.href}>
            <Link
              href={link.href}
              className="group flex items-center gap-2 rounded-md p-2 text-[13px] font-semibold tracking-tight text-foreground transition-colors hover:bg-white hover:text-foreground hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-1"
              onClick={onNavigate}
            >
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-none border border-black/8 bg-white text-primary shadow-sm transition-colors group-hover:border-primary/20 group-hover:text-primary">
      ...
      <div className="my-1.5 flex flex-1 flex-col justify-center rounded-none border border-black/6 bg-white p-2.5">
        <p className="text-[11px] font-semibold leading-snug text-foreground">Your GEO workspace</p>
    ...
    <>
      ...
      <ul role="list" className="flex flex-col gap-0.5">
        {hints.map(({ icon: Icon, label, hint }) => (
          <li
            key={label}
            className="flex items-start gap-2 rounded-md p-2 text-xs font-semibold tracking-tight text-foreground"
          >
            <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-none border border-black/8 bg-white text-primary shadow-sm">
    ...
    <div className="relative z-20 flex flex-1 items-center justify-end lg:justify-center">
      ...
      >
        <nav className="flex items-center gap-0.5 font-sans" aria-label="Primary">
            ...
            return (
              <div key={key} className="relative" onMouseEnter={() => openMenu(key)}>
                <button
                  ...
                  className={cn(
                    "inline-flex items-center gap-0.5 rounded-md px-3 py-2 text-sm font-medium tracking-tight transition-colors",
                    "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-1",
                    isOpen
          ...
          <Link
            href="/pricing"
            className="rounded-md px-3 py-2 text-sm font-medium tracking-tight text-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-1"
          >
            Pricing
          </Link>
        </nav>
          ...
          {open ? (
            ...
            >
              <motion.div
                ...
                className="rounded-none border-2 bg-white p-2 font-sans shadow-[0_20px_50px_-12px_rgba(15,23,42,0.18)]"
              ...
              >
                {open === "resources" ? (
                  ...
                  <motion.div
                    className="grid gap-2 rounded-none border border-black/6 sm:grid-cols-2 sm:gap-x-3 sm:gap-y-1"
                    variants={{
      ...
      <div className="hidden shrink-0 items-center gap-2 lg:flex">
        {isAuthenticated ? (
          ...
          <>
            <Button asChild variant="link" className="px-4 text-foreground hover:text-foreground">
              <Link href="/sign-in">Log In</Link>
            </Button>
            <Button variant={"default"} asChild className={cn(LANDING_PRIMARY_CTA_CLASS, "px-4")}>
      ...
      <AnimatePresence>
        {mobileOpen ? (
          <>
            ...
            />
            <motion.div
              className="absolute right-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] rounded-none border border-black/10 bg-white p-3 shadow-[0_20px_50px_-12px_rgba(15,23,42,0.22)] lg:hidden"
              ...
              <nav className="flex flex-col" aria-label="Mobile primary">
                {(Object.keys(MENUS) as MegaKey[]).map((key) => (
                  <details key={key} className="group border-b border-black/8 py-1 last:border-b-0">
                    <summary className="flex cursor-pointer list-none items-center justify-between rounded-md px-2 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted">
                      {MENUS[key].label}
                    ...
                    <div className="flex flex-col gap-1 px-2 pb-2 pt-1">
                      {MENUS[key].items.map((item) => (
                        <Link
                          key={`${key}-${item.title}`}
                          href={item.href}
                          className="rounded-md px-2 py-1.5 text-sm text-foreground hover:bg-muted hover:text-foreground"
                          onClick={() => setMobileOpen(false)}
                        >
                          {item.title}
                        </Link>
                ...
                <Link
                  href="/pricing"
                  className="mt-2 rounded-md px-2 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted"
                  onClick={() => setMobileOpen(false)}
                >
                  Pricing
                </Link>
