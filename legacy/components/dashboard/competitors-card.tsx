...
import type { Competitor, PageScore } from "@legacy/lib/api/analyzer";

const CORAL = "#e04a3d"; // matches the --primary token
...
function BrandLogo({ name, url, size = 24 }: { name: string; url?: string; size?: number }) {
  ...
  const palettes = [
    ...
    "from-[var(--feature-violet)] to-[var(--feature-violet)]",
    "from-info to-info",
...
function ScoreComparisonLines({ ranked }: { ranked: Row[] }) {
  ...
  function TooltipContent({ active, payload, label }: any) {
    ...
    return (
      <div className="min-w-[210px] rounded-xl border border-white/10 bg-foreground p-3 shadow-2xl">
            ...
            return (
              <div key={p.dataKey} className="flex items-center gap-2">
                ...
                {src ? (
                  ...
                  <span className="h-3.5 w-3.5 shrink-0 rounded-sm bg-foreground text-center text-[10px] leading-[14px] text-white">
                    {initialsOf(r?.name ?? "")}
                  </span>
                )}
        ...
        {top.map((r) => {
          ...
          const inner = (
            <span className="flex items-center gap-1.5">
              ...
              {r.isYou && <span className="text-[10px] text-muted-foreground">(you)</span>}
            </span>
          );
          return href ? (
            <a
...
export function CompetitorsCard({
  ...
  return (
    <div className="col-span-12 overflow-hidden rounded-xl border border-border bg-card">
      {/* Header */}
      ...
      {hasData ? (
            ...
            <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-1">
              ...
              <div
                className={cn(
                  ...
                  isLeading ? "border-success/30 bg-success/5" : "border-primary/20 bg-primary/5",
                )}
              >
                <div className="flex items-center justify-between">
                  ...
                  {isLeading && <Crown className="h-3.5 w-3.5 text-success" aria-hidden />}
                </div>
                <div className="mt-1 flex items-baseline gap-1.5">
                  <span
                    className={cn(
                      "text-3xl font-bold tabular-nums",
                      isLeading ? "text-success" : "text-primary",
                    )}
                  >
                    #{yourRank ?? ","}
                  </span>
                ...
                {isLeading ? (
                  <p className="mt-1 text-[11px] font-medium text-success">You're leading 🎉</p>
                ) : leader ? (
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    <span
                      className={cn(
                        "font-semibold",
                        roundedGap > 0 ? "text-destructive" : "text-success",
                      )}
                    >
              ...
              {/* Top 3 */}
              <div className="relative col-span-2 overflow-hidden rounded-xl border border-border bg-card p-3.5 lg:col-span-1">
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                    Top 3
                  </p>
          ...
          <div className="lg:col-span-8">
            <div className="flex h-full flex-col rounded-xl border border-border bg-card p-4">
              <div className="mb-2 flex items-start justify-between">
                <div>
                  <p className="text-xs font-semibold text-foreground">Score Comparison</p>
        ...
        <div className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
          ...
          <Link
            href={`/dashboard/${slug}/competitors`}
            className="mt-1 inline-flex items-center gap-1 rounded-full bg-primary px-3.5 py-1.5 text-xs font-semibold text-white shadow-sm transition-all hover:shadow-md"
          >
