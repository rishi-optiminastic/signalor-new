...
export function VisibilityByPlatformCard({
  ...
  return (
    <div className="col-span-4 flex h-full min-h-0 flex-col rounded-xl border border-border bg-card p-4">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-2 grid-rows-2 gap-2">
          {visibilityCards.map((card) => {
            ...
            return (
              <div
                key={card.label}
                className="relative flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-muted/30 p-2.5"
              >
                <div className="flex min-h-0 flex-1 flex-col">
                  <div className="mb-2 flex shrink-0 items-center justify-between">
                    <p className="text-xs font-medium text-foreground/80">{card.label}</p>
                    <span className="rounded-full border border-border bg-background p-0.5">
                      {card.icon}
                    </span>
                  </div>
                  <p
