...
export function AiEngineProbesCard({ sentiment }: { sentiment: DashboardSentiment | null }) {
  ...
  if (!sentiment || sentiment.aiTotal < 1) {
    return (
      <div className="col-span-12 bg-white rounded-xl p-5 border border-border shadow-sm flex flex-col">
        ...
        <div className="flex items-center gap-4 mt-3">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-muted-foreground">
              Mentioned{" "}
              <span className="font-bold text-foreground">{Math.round(mentionPct * 100)}%</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5">
