...
const PILLAR_STYLES: Record<ActionDef["pillar"], string> = {
  ...
  Content: "border-info/40 bg-info/10 text-info dark:text-info",
  Structural:
    "border-[var(--feature-violet)]/40 bg-[var(--feature-violet)]/10 text-[var(--feature-violet)] dark:text-[var(--feature-violet)]",
};

const IMPACT_STYLES: Record<ActionDef["impact"], string> = {
  high: "border-success/40 bg-success/10 text-success dark:text-success",
  medium: "border-warning/40 bg-warning/10 text-warning dark:text-warning",
              ...
              <div className="flex flex-col gap-2 px-3 py-2.5 sm:flex-row sm:items-center sm:gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-1">
                    <span
                      className={`rounded border px-1.5 py-px text-[10px] font-semibold uppercase ${PILLAR_STYLES[a.pillar]}`}
                    >
                      {a.pillar} · {a.pillarWeight}
                    </span>
                    <span
                      className={`rounded border px-1.5 py-px text-[10px] font-semibold uppercase ${IMPACT_STYLES[a.impact]}`}
                    >
                      {a.impact} impact
                    </span>
                    <span className="rounded border border-border bg-card px-1.5 py-px text-[10px] text-muted-foreground">
                      {a.effort}
                    </span>
                  </div>
                </div>
                ...
                <div className="border-t border-border/60 bg-card px-3 py-3">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 text-[11px] font-medium text-foreground">
                      ...
                      Generated artifact
                      {result.cached ? (
                        <span className="rounded border border-border bg-muted/30 px-1.5 py-px text-[10px] uppercase text-muted-foreground">
                          Saved
                        </span>
                      ) : null}
                    </div>
