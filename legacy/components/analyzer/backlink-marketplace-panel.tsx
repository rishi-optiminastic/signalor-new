...
const STATUS_STYLES: Record<BacklinkOrderStatus, { label: string; cls: string }> = {
  ...
  pending_payment: {
    label: "Pay pending",
    cls: "bg-warning/10 text-warning border-warning/30 dark:text-warning",
  },
  queued: {
    label: "Queued",
  ...
  delivered: {
    label: "Delivered",
    cls: "bg-success/10 text-success border-success/30 dark:text-success",
  },
  rejected: {
    label: "Rejected",
    cls: "bg-destructive/10 text-destructive border-destructive/30 dark:text-destructive",
  },
  ...
  return (
    <div className="rounded-md border border-border bg-card p-3">
      ...
      {unpaidOrders.length > 0 ? (
        <div className="mt-2.5 flex items-start gap-2 rounded-md border border-warning/40 bg-warning/10 px-3 py-2">
          <CreditCard className="mt-0.5 size-4 shrink-0 text-warning dark:text-warning" />
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-[11px] leading-snug text-warning dark:text-warning/90">
            ...
            return (
              <div key={o.id} className="rounded-md border border-border bg-muted/15">
                <div className="flex min-w-0 items-center gap-2 px-2.5 py-1.5">
                  {o.status === "delivered" ? (
                    <CheckCircle2 className="size-3.5 shrink-0 text-success dark:text-success" />
                  ) : (
                  ...
                  <span
                    className={`rounded border px-1.5 py-px text-[10px] font-semibold ${s.cls}`}
                  >
                    {s.label}
                  </span>
                  {needsPayment ? (
          ...
          visibleProducts.map((p) => (
            ...
            >
              <div className="flex min-w-0 items-start gap-2">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-semibold text-foreground">{p.domain}</p>
              ...
              <div className="flex flex-wrap items-center gap-1">
                <span className="rounded border border-border bg-card px-1.5 py-px text-[10px] font-medium text-foreground">
                ...
                {p.do_follow ? (
                  <span className="rounded border border-border bg-muted/40 px-1.5 py-px text-[10px] text-muted-foreground">
                    do-follow
                  </span>
                ) : null}
                <span className="rounded border border-border bg-muted/30 px-1.5 py-px text-[10px] text-muted-foreground">
                  {p.lead_time_days}d
                </span>
