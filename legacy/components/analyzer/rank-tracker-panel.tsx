  ...
  return (
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div className="min-w-0">
        <p className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">Ranking</p>
  ...
  return (
    <div className="flex flex-col gap-2">
      {queries.map((q) => {
        ...
        const sevTone =
          sev === "green" ? "bg-success" : sev === "amber" ? "bg-warning" : "bg-destructive";
        ...
        {result.is_brand_mentioned || result.competitors_mentioned.length > 0 ? (
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            {result.is_brand_mentioned ? (
              <span className="inline-flex items-center rounded-md border border-success/40 bg-success/10 px-1.5 py-0.5 text-[10px] font-semibold text-success">
                Your brand
              </span>
            ) : null}
            {result.competitors_mentioned.map((c) => (
              <span
                key={c}
                className="inline-flex items-center rounded-md border border-warning/40 bg-warning/10 px-1.5 py-0.5 text-[10px] font-semibold text-warning"
              >
                {c}
              </span>
            ))}
...
function BrandCitationSummary({ query, mode }: { query: RankQuery; mode: RankTrackerMode }) {
  ...
  if (!stats.length) {
    return (
      <div className="border-b border-border/50 bg-muted/30 px-4 py-3 text-xs text-muted-foreground">
...
function BrandStatChip({ s }: { s: BrandStats }) {
  ...
  const borderClass =
    tone === "emerald" ? "border-success/40 bg-success/10" : "border-warning/40 bg-warning/10";
  const textClass = tone === "emerald" ? "text-success" : "text-warning";
  return (
    <div
      className={cn(
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {result.is_brand_mentioned ? (
            ...
            <span className="inline-flex items-center rounded-md border border-border/60 bg-muted px-1.5 py-0.5 text-[10px] font-semibold text-muted-foreground">
              Brand absent
            </span>
          )}
        </div>
