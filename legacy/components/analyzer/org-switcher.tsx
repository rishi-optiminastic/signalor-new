...
import { ChevronDown, Check, Plus, Trash2, Users } from "@legacy/components/icons";
import { createPortal } from "react-dom";
import { useOrgStore } from "@legacy/lib/stores/org-store";
import type { Organization } from "@legacy/lib/api/organizations";
import { deleteOrganization } from "@legacy/lib/api/organizations";
import { useAccountType } from "@legacy/lib/hooks/use-account-type";
...
export function OrgSwitcher({ onOrgChange }: OrgSwitcherProps) {
  ...
  function handleDeleteClick(e: React.MouseEvent<HTMLButtonElement>, org: Organization) {
    e.preventDefault();
    e.stopPropagation();
    setError("");
    setOpen(false);
  ...
  const deleteNameMatches =
    orgToDelete != null && deleteNameConfirm.trim() === orgToDelete.name.trim();
  ...
  return (
    <>
      <div className="relative">
        ...
        >
          <span className="max-w-[160px] truncate">{activeOrg?.name ?? "Select project"}</span>
              ...
              <ul className="py-1">
                ...
                <li className="border-t border-border/50 mt-1 pt-1">
                  <button
                    onClick={() => {
                      setOpen(false);
                      // Always go through onboarding to actually CREATE a new
                      // project. Routing to /dashboard would just bounce a user
                      // who already has projects back to their existing
                      // dashboard (looks like "nothing happens"). The onboarding
                      // flow adds the project, or shows the plan-limit / upgrade
                      // gate when the user is at their cap (individuals = 1).
                  ...
                  >
                    ...
                    {isAgency ? "Add client brand" : "New project"}
                  </button>
                </li>
                {isAgency && (
                  <li>
                    <button
                      onClick={() => {
                        setOpen(false);
                        router.push(routes.agencyWorkspace);
                      }}
                      className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
                    >
