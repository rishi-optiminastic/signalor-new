...
import { ExternalLink, Link2, Loader2, AlertCircle } from "@legacy/components/icons";
import {
  getPromptBacklinks,
  type BacklinkRow,
  ...
  return (
    <div className="border-t border-border bg-muted/20 px-4 py-5">
      ...
      <p className="mt-2 text-[10px] leading-snug text-muted-foreground">
        ...
        {data.api_error ? <span className="text-warning"> · {data.api_error}</span> : null}
      </p>
    </div>
  );
}
    ...
    <tr className={cn("border-b border-border last:border-b-0", row.is_brand && "bg-primary/5")}>
      <td className="px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          {row.is_competitor && (
            <span className="rounded bg-warning/15 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-warning dark:text-warning">
              Competitor
            </span>
          )}
        </div>
      ...
      <td className="px-3 py-2 text-right tabular-nums">
        {gap === null || !row.has_data ? (
          ...
          <span className="font-medium text-warning dark:text-warning">+{formatNumber(gap)}</span>
        ) : gap < 0 ? (
          <span className="font-medium text-success dark:text-success">{formatNumber(gap)}</span>
        ) : (
