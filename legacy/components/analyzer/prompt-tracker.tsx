...
import { ENGINE_LOGO_SRC } from "@legacy/lib/engines";
...
const SENTIMENT_STYLES: Record<Sentiment, { pill: string; dot: string }> = {
  positive: {
    pill: "bg-success/10 text-success dark:text-success border-success/20",
  ...
  negative: {
    pill: "bg-destructive/8 text-destructive dark:text-destructive border-destructive/20",
        ...
        <div className="flex w-full min-w-0 flex-nowrap items-center justify-start gap-2 overflow-x-auto overflow-y-visible py-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:w-auto sm:max-w-[min(100%,52rem)] sm:justify-end lg:max-w-none">
          {tracks.length > 0 && onRecheckAll ? (
            <Button
              ...
              className="h-9 min-h-9 shrink-0 gap-1.5 border-border/80 bg-white px-2.5 text-xs font-medium text-foreground shadow-sm hover:bg-muted disabled:opacity-50 dark:bg-white dark:text-foreground dark:hover:bg-muted"
          ...
          {tracks.length > 0 ? (
            <>
              <div className="relative min-w-42 max-w-56 shrink-0 px-0.5 sm:min-w-48 sm:max-w-xs md:max-w-64">
                ...
                {search ? (
                  <button
                    type="button"
                    onClick={() => setSearch("")}
                    className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-0.5 text-muted-foreground hover:bg-muted hover:text-foreground dark:hover:bg-muted"
              ...
              <Select value={filterLabel} onValueChange={(v) => setFilterLabel(v as FilterLabel)}>
                <SelectTrigger
                  size="default"
                  className="h-9 min-h-9 w-34 shrink-0 border border-border/80 bg-white py-0 text-sm text-foreground shadow-sm sm:w-36 dark:bg-white dark:text-foreground dark:hover:bg-muted"
                >
              ...
              <Select value={sortKey} onValueChange={(v) => setSortKey(v as SortKey)}>
                <SelectTrigger
                  size="default"
                  className="h-9 min-h-9 w-30 shrink-0 border border-border/80 bg-white py-0 text-sm text-foreground shadow-sm sm:w-32 dark:bg-white dark:text-foreground dark:hover:bg-muted"
                >
          ...
          {tracks.length > 0 && promptFiltersActive ? (
            <Button
              ...
              variant="outline"
              size="default"
              className="h-9 min-h-9 shrink-0 border-border/80 bg-white px-2.5 text-xs text-muted-foreground shadow-sm hover:bg-muted dark:bg-white dark:text-foreground dark:hover:bg-muted"
              onClick={() => {
                setSearch("");
                setFilterLabel("All");
                setSortKey("score");
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5">
                      ...
                      {track.is_custom && (
                        <span className="rounded border border-border/40 px-1 py-px text-[10px] text-muted-foreground">
                          Custom
                        </span>
                      )}
                    </div>
                  ...
                  {/* Chevron (right) — matches Tasks table */}
                  <span
                          ...
                          <div className="space-y-3 p-3 sm:p-3.5">
                              ...
                              {sentTotal > 0 && (
                                <div className="rounded-md border border-border bg-card p-3">
                                  ...
                                  <div className="mt-2 flex h-1 overflow-hidden rounded-full bg-muted">
                                    {posCount > 0 && (
                                      <div
                                        className="bg-success/80 dark:bg-success/70"
                                        style={{
                                    ...
                                    {negCount > 0 && (
                                      <div
                                        className="bg-destructive/70 dark:bg-destructive/60"
                                        style={{
                                          ...
                                          {rows.map(({ eng, r }) => {
                                            ...
                                            return (
                                              ...
                                              >
                                                ...
                                                {r.brand_mentioned ? (
                                                  <span className="rounded border border-success/40 bg-success/10 px-1 py-px text-[10px] font-semibold text-success dark:text-success">
                                                    Mentioned
                                                  </span>
                                                ) : (
                                                ...
                                                {r.rank_position > 0 ? (
                                                  <span className="rounded border border-border bg-card px-1 py-px text-[10px] font-medium tabular-nums text-foreground">
                                                    #{r.rank_position}
                                                  </span>
                                                ) : null}
                                                <span
                                          ...
                                          {top.map(([domain, info]) => (
                                            ...
                                            >
                                              ...
                                              {info.isBrand ? (
                                                ...
                                                <span className="rounded border border-warning/40 bg-warning/10 px-1 py-px text-[10px] font-semibold text-warning dark:text-warning">
                                                  Competitor
                                                </span>
                                              ) : null}
                                              <span className="text-[10px] tabular-nums text-muted-foreground">
                                      ...
                                      >
                                        ...
                                        >
                                          <div className="mb-1 flex flex-wrap items-center gap-1.5">
                                            ...
                                            <span
                                              className={`rounded border px-1 py-px text-[10px] capitalize ${SENTIMENT_STYLES[r.sentiment as Sentiment]?.pill}`}
                                            >
                                              {r.sentiment}
                                            </span>
                                            {r.rank_position > 0 && (
                            ...
                            <div className="mt-2 flex items-center justify-between gap-3 border-t border-border/50 pt-4">
                              ...
                              <button
                                ...
                                className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-primary/30 px-3 text-xs font-medium text-primary transition hover:bg-primary/10"
                              >
            ...
            >
              ...
              >
                <div className="px-6 pt-6 pb-4">
                  ...
                  {deleteError && <p className="mt-3 text-[11px] text-primary">{deleteError}</p>}
                </div>
                ...
                <div className="flex items-center justify-end gap-2 border-t border-border/60 bg-muted/20 px-6 py-3">
                  ...
                  <button
                    ...
                    disabled={isDeleting}
                    className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-primary px-3 text-xs font-medium text-white transition hover:brightness-110 disabled:opacity-50"
                  >
                    {isDeleting ? (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold text-foreground">Backlinks for this prompt</p>
          ...
          top.map((p) => (
            ...
            >
              <div className="flex min-w-0 items-baseline justify-between gap-2">
                <span className="truncate text-xs font-semibold text-foreground">{p.domain}</span>
              ...
              <div className="flex flex-wrap items-center gap-1">
                {p.domain_authority != null ? (
                  <span className="rounded border border-success/30 bg-success/10 px-1.5 py-px text-[10px] font-semibold text-success dark:text-success">
                    DA {p.domain_authority}
                  </span>
                ) : null}
                <span className="rounded border border-border bg-muted/40 px-1.5 py-px text-[10px] font-medium text-muted-foreground">
              ...
              <ul className="space-y-1.5">
                {[1, 2, 3].map((pos) => (
                  <li key={pos} className="flex min-w-0 items-center gap-1.5">
                    <span className="inline-flex size-4 shrink-0 items-center justify-center rounded-sm bg-muted/80 text-[10px] font-semibold tabular-nums text-muted-foreground/70">
                      {pos}
                    </span>
                    <div
