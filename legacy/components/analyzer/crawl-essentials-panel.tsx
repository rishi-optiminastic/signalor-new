...
export function CrawlEssentialsPanel({ email, runId, analyzedUrl }: CrawlEssentialsPanelProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const message = err?.response?.data?.error || "Failed to load AI crawl essentials status.";
      setError(message);
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Validate `llms.txt`, `robots.txt`, and `sitemap.xml` and get AI-guided improvement
              actions.
            </p>
            {data && (
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-lg border border-border/60 bg-background/60 p-3">
                ...
                <p className="text-2xl font-semibold text-success">{summary.good}</p>
              </div>
              <div className="rounded-lg border border-border/60 bg-background/60 p-3">
                <p className="text-xs text-muted-foreground">Needs Improvement</p>
                <p className="text-2xl font-semibold text-warning">{summary.improve}</p>
              </div>
              <div className="rounded-lg border border-border/60 bg-background/60 p-3">
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">Score {file.score}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button
                          type="button"
