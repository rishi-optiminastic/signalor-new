...
import type { Competitor } from "@legacy/lib/api/analyzer";
import { deleteCompetitor, getRankings } from "@legacy/lib/api/analyzer";
import { LogoImg } from "@legacy/components/dashboard-v2/ui/LogoImg";
import { engineMeta } from "@legacy/components/dashboard-v2/data/constants";
import {
  Select,
  SelectContent,
  SelectItem,
        ...
        createPortal(
          ...
          >
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-2 text-[13px] font-medium text-destructive transition hover:bg-destructive/10"
            >
      ...
      <div className="w-full max-w-sm rounded-xl border border-border bg-white p-6 shadow-2xl">
        ...
        <div className="mt-5 flex justify-end gap-2.5">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={deleting}
            className="inline-flex h-9 items-center gap-1.5 rounded-md bg-destructive px-4 text-[13px] font-semibold text-white transition hover:bg-destructive disabled:opacity-60"
          >
            {deleting ? (
              <>
...
export function CompetitorTable({
  ...
  // Which AI models each competitor actually ranks in (from the rankings API).
  const { data: rankingsData } = useQuery({
    queryKey: ["v2-rankings", slug],
    queryFn: () => getRankings(slug as string),
    enabled: !!slug,
    staleTime: 5 * 60 * 1000,
  });
  const enginesByName = useMemo(() => {
    const m = new Map<string, string[]>();
    for (const r of rankingsData?.rows ?? []) m.set(r.company.trim().toLowerCase(), r.engines);
    return m;
  }, [rankingsData]);
  const brandEngines = rankingsData?.rows.find((r) => r.is_you)?.engines ?? [];
  const enginesFor = (r: BrandRow): string[] =>
    r.isMine ? brandEngines : (enginesByName.get(r.name.trim().toLowerCase()) ?? []);
                ...
                {rows.map((r) => {
                  ...
                  return (
                    ...
                    >
                            ...
                            {!r.locked && !r.isMine && !r.scored && (
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <span className="inline-flex h-4 w-4 shrink-0 cursor-default items-center justify-center rounded-full bg-destructive/10">
                      ...
                      {/* Models — which AI engines this brand actually ranks in */}
                      <td className="px-3 py-3">
                        {(() => {
                          const engs = enginesFor(r);
                          if (!engs.length)
                      ...
                      <td className="px-3 py-3">
                        {r.isMine ? (
                          ...
                          >
                            <SelectTrigger
                              className={cn(
                                "h-9 w-full border border-border/80 bg-white text-[13px] font-medium text-foreground shadow-sm focus:ring-0 focus:border-border dark:bg-white dark:text-foreground dark:hover:bg-muted",
...
function ScoreChip({ value }: { value: number }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex min-w-[40px] justify-center rounded-md px-2 py-0.5 text-xs font-semibold",
        tint,
      )}
    >
      {value}
