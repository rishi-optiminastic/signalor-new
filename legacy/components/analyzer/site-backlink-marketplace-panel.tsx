    ...
    <div className="rounded-xl border border-border bg-card p-4">
          ...
          >
            ...
            <div className="flex flex-wrap gap-1">
              {provider.linkTypes.map((lt) => (
                <span
                  key={lt}
                  className="rounded-full bg-muted/60 px-2 py-0.5 text-[10px] font-medium text-foreground/80"
                >
                  {lt}
                </span>
              ))}
      ...
      {visibleProviders.length === 0 ? (
        <div className="mt-3 rounded-md border border-dashed border-border/70 bg-muted/15 px-3 py-8 text-center">
          ...
          <p className="text-xs font-medium text-foreground">No providers match this filter</p>
