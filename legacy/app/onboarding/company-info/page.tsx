...
import { useSession, authClient } from "@legacy/lib/auth-client";
import { useOnboardingStore, SIGNUP_DETAILS_KEY } from "@legacy/lib/stores/onboarding-store";
import { Button } from "@legacy/components/ui/button";
import { CardContent, CardFooter } from "@legacy/components/ui/card";
import { Input } from "@legacy/components/ui/input";
import { Label } from "@legacy/components/ui/label";
import { createOrganization } from "@legacy/lib/api/organizations";
import { startAnalysis } from "@legacy/lib/api/analyzer";
import { getSubscriptionStatus } from "@legacy/lib/api/payments";
import { getShopifyAuthUrl, connectWordPress } from "@legacy/lib/api/integrations";
import { getOrFetchOnboardingToken } from "@legacy/lib/api/onboarding-security";
import { useProjectUsage } from "@legacy/lib/hooks/use-project-usage";
import { ProjectLimitReached } from "@legacy/components/onboarding/project-limit-reached";
import { GithubConnectStep } from "@legacy/components/onboarding/github-connect-step";
import { TurnstileWidget } from "@legacy/components/onboarding/turnstile-widget";
import { useOnboardingUiStore } from "@legacy/lib/stores/onboarding-ui-store";
import { OnboardingStepper } from "@legacy/components/auth/onboarding-stepper";
import { config, routes, signalorWpPlugin } from "@legacy/lib/config";
import { ONBOARDING_DRAFT_KEY, storePendingAnalysisAfterPayment } from "@legacy/lib/internal-nav";
import { identifyUser, track } from "@legacy/amplitude";
...
const PANEL = "rounded-xl border border-black/8 bg-white shadow-md isolate";

const ERR_BOX =
  "rounded-md border border-destructive/20 bg-destructive/5 px-3 py-2 text-xs font-medium text-destructive";

const STATUS_BOX =
  "flex items-center gap-2 rounded-md border border-black/6 bg-muted/35 px-3 py-2 text-xs text-muted-foreground";
...
function IntegrationConnectBanner({ platform }: { platform: Platform }) {
  ...
  return (
    <div className="mb-5 flex items-center justify-center gap-3">
      {/* Platform logo — left */}
      <div
        className="flex h-[60px] w-[60px] shrink-0 items-center justify-center rounded-2xl border border-black/8 shadow-md"
...
function BrandConnectBanner({ siteUrl }: { siteUrl: string }) {
  ...
  return (
    <div className="mb-4 flex items-center justify-center gap-3">
      <div className="flex h-[60px] w-[60px] shrink-0 items-center justify-center rounded-2xl border border-black/8 bg-white shadow-md">
        {domain && !failed ? (
          <img
...
function SiteFavicon({ url }: { url: string }) {
  ...
  return (
    <img
      ...
      width={14}
      height={14}
      className="inline-block size-3.5 shrink-0 rounded-sm object-contain align-middle"
      onError={() => setFailed(true)}
    />
  );
}
...
export default function CompanyInfoPage() {
  ...
  // Publish the current step so the onboarding layout's right-rail can switch
  // illustrations (marketing panel on "company", integrations grid after).
  useEffect(() => {
    useOnboardingUiStore.setState({ step });
  }, [step]);
  const [companyName, setCompanyName] = useState("");
  // Name captured on the dedicated sign-up step; carried here for the welcome
  // email and to persist onto the auth user (agency Google path).
  ...
  // Project-limit gate. `usage` drives the pre-flight block; `limitHit` is a
  // defensive fallback if a 403 ever slips through on submit.
  const usage = useProjectUsage(session?.user?.email);
  const [limitHit, setLimitHit] = useState(false);
  const atProjectLimit = limitHit || (usage.loaded && usage.atLimit);
  ...
  // Carry over the user's name captured on the dedicated agency sign-up step.
  // The onboarding store survives client-side navigation (email path); the
  // sessionStorage mirror survives the Google OAuth redirect. The typed name
  // wins over the Google-provided session name, and is persisted to the auth
  // user so it shows across the app (welcome email, top bar). The agency name
  // is NOT prefilled here — this field is the brand/project name, and the
  // agency name is saved separately on the account. Runs once.
  const detailsAppliedRef = useRef(false);
  const sessionUserName = session?.user?.name;
  useEffect(() => {
    if (detailsAppliedRef.current) return;

    let typedName = useOnboardingStore.getState().userName;
    if (!typedName && typeof window !== "undefined") {
      try {
        const raw = sessionStorage.getItem(SIGNUP_DETAILS_KEY);
        if (raw) typedName = JSON.parse(raw).userName || "";
      } catch {
        /* ignore */
      }
    }
    ...
    if (typedName) {
      ...
      if (typedName !== (sessionUserName ?? "")) {
        authClient.updateUser({ name: typedName }).catch(() => {});
      }
    } else if (sessionUserName) {
      setUserName((prev) => prev || sessionUserName);
    }

    // Mark applied only once we've seen a resolved session (so the Google name
    // is available to compare against); until then, keep trying.
  ...
  // The project-limit gate is handled by `useProjectUsage` above and rendered
  // before the steps — for ALL users (paid or not), no /pricing redirect.
  ...
  async function handleUrlNext(e: React.FormEvent) {
    ...
    try {
      ...
      // Project-limit 403 (defensive — the pre-flight gate normally catches
      // this first). Show the limit-reached screen instead of redirecting to
      // /pricing, which is wrong for subscribed users at quota.
  ...
  async function handleLaunch() {
    ...
    try {
      ...
      import("@legacy/lib/api/client").then(({ apiClient }) => {
        apiClient
          .post("/email/welcome/", {
            email: session.user.email,
            first_name: (userName || session.user.name || "").trim().split(" ")[0],
  ...
  // Block project creation up front while we confirm the user's quota, then —
  // if they're already at the limit — render the limit-reached screen instead
  // of any step so no project details can be entered.
  if (usage.loading) {
    return (
      <CardContent
        className="flex flex-col items-center justify-center gap-3 px-0 py-14"
        aria-busy="true"
      >
        <Loader2 className="h-6 w-6 animate-spin text-muted-foreground" aria-hidden />
        <p className="text-xs text-muted-foreground">One moment</p>
      ...
      <CardContent className="space-y-4 px-0 pt-5">
        ...
        {step === "platform" && (
          <div className="space-y-3">
            <div className="space-y-3">
              {[
                ...
                <button
                  ...
                  className="flex w-full items-center gap-4 rounded-xl border border-black/8 bg-white p-4 text-left shadow-md transition hover:border-black/12 hover:bg-muted/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 active:scale-[0.99]"
                >
                  <div
            ...
            <Button
              type="button"
              variant="outline"
              className="h-9 w-full rounded-md border-border bg-white text-[13px] font-medium"
          ...
          <form onSubmit={handleUrlNext} className="space-y-5">
            ...
            <div className="space-y-2">
              <Input
                ...
                required
                autoFocus
                className={`h-9 rounded-md bg-white text-[13px] ${shopDomainErr ? "border-destructive focus-visible:ring-destructive/20" : "border-border"}`}
              />
            ...
            <div className="flex gap-2">
              <Button
                type="button"
                variant="outline"
                className="h-9 flex-1 rounded-md border-border bg-white text-[13px] font-medium"
                onClick={() => {
                  setStep("platform");
                  setOrgId(null);
                  setError("");
          ...
          <form onSubmit={handleUrlNext} className="space-y-5">
            ...
            <div className="space-y-2">
              <Input
                ...
                required
                autoFocus
                className={`h-9 rounded-md bg-white text-[13px] ${wpUrlErr ? "border-destructive focus-visible:ring-destructive/20" : "border-border"}`}
              />
        ...
        {step === "install" && platform === "shopify" && (
          <div className="space-y-4">
            ...
            <div className="text-center">
              <p className="text-sm font-semibold text-foreground">Install Signalor on Shopify</p>
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                Connecting{" "}
            ...
            <div className="flex gap-2">
              ...
              <Button
                type="button"
                variant="outline"
                className="h-9 flex-1 rounded-md border-border bg-white text-[13px] font-medium text-muted-foreground hover:text-foreground"
        ...
        {step === "install" && platform === "wordpress" && (
          <div className="space-y-4">
            ...
            <div className="text-center">
              <p className="text-sm font-semibold text-foreground">Install the Signalor plugin</p>
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              ...
              {wpStep >= 3 && (
                <div className={`${PANEL} p-4`}>
                  ...
                  <Input
                    ...
                    className="h-9 rounded-md border-border bg-white font-mono text-[13px]"
                  />
                </div>
              )}
            </div>
        ...
        {step === "install" && platform === "framer" && (
          <div className="space-y-4">
            <BrandConnectBanner siteUrl={wpSiteUrl} />
            <div className="text-center">
              <p className="text-sm font-semibold text-foreground">Open the Signalor plugin</p>
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                Add the plugin URL in Framer to verify your site connection.
              </p>
            </div>
            <div className={`${PANEL} p-4`}>
              <ol className="mb-3 list-decimal space-y-1.5 pl-4 text-xs leading-relaxed text-muted-foreground">
                <li>
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  className="h-9 flex-1 rounded-md border-border bg-white font-mono text-xs"
                  onFocus={(e) => e.target.select()}
                />
                <Button
                  type="button"
                  variant="outline"
                  className="h-9 rounded-md border-border bg-white px-3 text-xs font-medium"
                  onClick={async () => {
                    try {
        ...
        {step === "install" && platform === "nextjs" && (
          <div className="space-y-3">
            <div className="text-center">
              <p className="text-[15px] font-semibold text-foreground">Connect your GitHub repo</p>
              <p className="mt-0.5 text-[12px] leading-relaxed text-muted-foreground">
                Signalor opens fix PRs on your repo — you review &amp; merge.
              </p>
            </div>
            <GithubConnectStep
              email={session?.user?.email ?? ""}
          ...
          <div className="space-y-3">
            ...
            {loadingPrompts ? (
              <div className="flex flex-col items-center gap-3 py-12">
                ...
                <p className="text-xs text-muted-foreground">Generating prompts…</p>
              </div>
            ) : (
              <>
                <div className={PANEL}>
                      ...
                      {editingIdx === idx ? (
                        ...
                        <div className="flex h-full items-center gap-2 px-4">
                          <Input
                            ...
                            className="h-8 flex-1 rounded-md border-border bg-white text-[13px]"
                            autoFocus
                            onKeyDown={(e) => {
                          ...
                          <button
                            ...
                            className="px-2 text-xs font-medium text-foreground hover:underline"
                          >
                            Save
                          </button>
                        </div>
                ...
                <Button
                  ...
                  className="mb-2 flex h-9 w-full items-center gap-2 rounded-md border-dashed border-border bg-white text-[13px] text-muted-foreground hover:text-foreground disabled:opacity-30"
                >
            ...
            <div className={`mb-2 overflow-hidden ${PANEL}`}>
              {[
                ...
                >
                  ...
                  <span
                    className={`text-xs font-medium ${r.k === "App Installed" && !appInstalled ? "text-warning" : "text-foreground"}`}
                  >
                    {r.v}
                  </span>
                </div>
