...
import { ArrowLeft, Home, RefreshCw } from "@legacy/components/icons";
...
export default function ErrorBoundary({
  ...
  // TEMP DIAGNOSTIC: surface the real error so we can find what's throwing on
  // the dashboard. Remove once the loop is fixed.
  useEffect(() => {
        ...
        <div className="relative z-10 mx-auto w-full max-w-lg text-center">
          ...
          <p className="mx-auto mt-5 max-w-md text-base leading-relaxed text-muted-foreground">
            ...
            {process.env.NODE_ENV === "development" && (error.message || error.stack) ? (
              <span className="mt-3 block max-h-64 overflow-auto rounded-lg border border-border bg-muted/50 p-3 text-left font-mono text-[11px] leading-relaxed text-foreground whitespace-pre-wrap">
