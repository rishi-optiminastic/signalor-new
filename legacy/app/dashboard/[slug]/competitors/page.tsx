...
} from "@legacy/components/analyzer/competitor-table";
...
import { CompetitorsSkeleton } from "@legacy/components/dashboard/skeletons";
import { addCompetitor } from "@legacy/lib/api/analyzer";
import { useSession } from "@legacy/lib/auth-client";
import { identifyUser, track } from "@legacy/amplitude";
import { Globe, Loader2, Plus, Search, X } from "@legacy/components/icons";
...
function AddCompetitorModal({
  ...
  // Open as a native top-layer modal so it sits above the entire app
  // (sidebar, topbar, transformed ancestors) — no z-index/stacking battles.
  useEffect(() => {
  ...
  return (
    <dialog
      ...
      className="m-auto w-full max-w-md bg-transparent p-0 backdrop:bg-black/40 backdrop:backdrop-blur-[2px]"
    >
      <div className="w-full rounded-xl border border-border bg-white p-6 shadow-2xl">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-foreground" htmlFor="comp-url">
              Website URL
            </label>
          ...
          {/* Brand Name */}
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-foreground" htmlFor="comp-name">
              Brand name
            </label>
            <input
              id="comp-name"
