      ...
      if (def?._type === "link" && def.href) {
        node = (
          <a
            ...
            className="font-medium text-warning underline underline-offset-2 hover:brightness-110"
          >
            {node}
          </a>
        );
...
function renderBlock(block: PTBlock, idx: number): React.ReactNode {
  ...
  if (block.listItem === "bullet") {
    return (
      <li
        key={block._key ?? idx}
        className="ml-5 list-disc text-sm leading-relaxed text-foreground/80"
      >
        {children}
      </li>
    );
  ...
  switch (block.style) {
    ...
    case "blockquote":
      return (
        <blockquote
          key={block._key ?? idx}
          className="my-6 border-l-4 border-warning/40 pl-5 text-sm italic leading-relaxed text-foreground/70"
        >
          {children}
        </blockquote>
      );
    default:
      return (
        <p key={block._key ?? idx} className="my-4 text-sm leading-relaxed text-foreground/80">
          {children}
        </p>
      );
  }
    ...
    <LandingMarketingShell>
      ...
      <section className="relative bg-background px-6 pb-12 pt-14 lg:px-12 lg:pb-14 lg:pt-16">
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          ...
          <p className="mt-4 text-base font-light leading-relaxed text-accent-foreground lg:text-base">
            {post.excerpt}
          </p>
        </div>
      </section>
            ...
            <div className="flex justify-start">
              {prevPost ? (
                <Link
                  ...
                  className="group inline-flex items-center gap-1.5 rounded-none border border-black/10 bg-white px-4 py-2 text-xs font-semibold text-foreground shadow-sm transition hover:bg-muted"
                >
